	...
	private class Controller {
		...
		private void okAction() {
			...
			if (endDate.isBefore(startDate)) {
				ConferenceAdministration.this.lblError.setText("End date cannot be before start date");
				return;
			}
			...
			for (Excursion excursion : ConferenceAdministration.this.arrExcursions) {
				Service.createExcursion(excursion.getName(), excursion.getDate(), excursion.getPrice(),
						this.conference);
			}
		...
		private void addExcursionAction() {
			...
			if (startDate == null || endDate == null) {
				ConferenceAdministration.this.lblError.setText("Set the dates before adding excursions");
				return;
			}

			Stage stage = (Stage) ConferenceAdministration.this.listExcursions.getScene().getWindow();
		...
		private void addHotelAction() {
			Hotel hotel = ConferenceAdministration.this.availableHotels.getSelectionModel().getSelectedItem();
			int index = ConferenceAdministration.this.availableHotels.getSelectionModel().getSelectedIndex();
			if (hotel == null) {
				return;
			}
		...
		private void removeHotelAction() {
			Hotel hotel = ConferenceAdministration.this.addedHotels.getSelectionModel().getSelectedItem();
			int index = ConferenceAdministration.this.addedHotels.getSelectionModel().getSelectedIndex();
			if (hotel == null) {
				return;
			}
