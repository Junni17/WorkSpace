...
public class Service
{
     ...
     */
    public static Conference createConference(String name, int price, LocalDate startDate,
            LocalDate endDate)
    {
     ...
     */
    public static Excursion createExcursion(String name, LocalDate date, int price,
     ...
     * Requires: name not empty.
     */
    public static Participant createParticipant(String name, String address, String city,
    ...
    {
        Enrollment enr = new Enrollment(dateOfArrival, dateOfDeparture, presentingTalk, participant,
                conference, hotel);
    ...
    public static String getExcursionInfo(Conference conference)
    {
        ...
        for (int i = 0; i < conference.getExcursions().size(); i++) {
            ...
            for (int j = 0; j < conference.getExcursions().get(i).getCompanions().size(); j++) {
                excursions += "Companion: "
                        + conference.getExcursions().get(i).getCompanions().get(j)
                        + ". Participant: " + conference.getExcursions().get(i).getCompanions()
                                .get(j).getEnrollment().getParticipant()
                        + "\n";
            }
        }
        return excursions;
    ...
    public static String getHotelsInfo(Conference conference)
    {
        ...
        for (int i = 0; i < conference.getHotels().size(); i++) {
            ...
            for (int j = 0; j < conference.getHotels().get(i).getEnrollments().size(); j++) {
                hotels += "Participant: "
                        + conference.getHotels().get(i).getEnrollments().get(j).getParticipant();
                if (conference.getHotels().get(i).getEnrollments().get(j).getCompanion() != null) {
                    hotels += " Companion: "
                            + conference.getHotels().get(i).getEnrollments().get(j).getCompanion();
                }
                hotels += "\n";
            }
        }
    ...
    public static void initStorage()
    {
        ...
        Hotel h2 = Service.createHotel("Ritz", 2000, 3000);
        h2.createExtra("WIFI", 75);
        h2.createExtra("Room service", 100);
