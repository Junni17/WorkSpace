...
public class EnrollmentAdministration extends Stage
{
    ...
    private ComboBox<Participant> cmbParticipant;
    ...
    private CheckBox companion, presentingTalk, hotel;
    ...
    private void initContent(GridPane pane)
    {
        ...
        this.cmbParticipant.setPromptText("Select a Participant");
        ...
        this.btnAddExcursion.setOnAction(event -> this.controller.addExcursionAction());
        ...
        this.btnRemoveExcursion.setOnAction(event -> this.controller.removeExcursionAction());

        // Hotels and Extras
        // -----------------------------------------------------------------------------------------

        this.hotel = new CheckBox("Hotel");
        pane.add(this.hotel, 0, 9);
        this.hotel.setOnMouseClicked(event -> this.controller.enableHotel(event));

        this.cmbHotel = new ComboBox<Hotel>();
        pane.add(this.cmbHotel, 1, 9);
        this.cmbHotel.setPromptText("Select a Hotel");
        ChangeListener<Hotel> listener = (ov, oldEmployee, newEmployee) -> this.controller
                .updateExtras();
        this.cmbHotel.getSelectionModel().selectedItemProperty().addListener(listener);
        this.cmbHotel.setDisable(true);
        ...
        Button btnDeleteExtra = new Button("Remove");
        pane.add(btnDeleteExtra, 2, 11);
        btnDeleteExtra.setPrefWidth(60);
        btnDeleteExtra.setOnAction(event -> this.controller.deleteExtraAction());

        // OK, cancel buttons
        // -----------------------------------------------------------------------------------------

        this.lblError = new Label();
        pane.add(this.lblError, 1, 12, 2, 1);
    ...
    private class Controller
    {
        ...
        // Creates an enrollment on OK button click
        private void okAction()
        {
            Participant participant = EnrollmentAdministration.this.cmbParticipant
            ...
            Hotel hotel = EnrollmentAdministration.this.cmbHotel.getSelectionModel()
                    .getSelectedItem();
            ...
            if (this.checkinput()) {
                ...
                this.enrollment = Service.createEnrollment(arrival, departure, presentingTalk,
                        participant, this.conference, hotel);

                participant.addEnrollment(this.enrollment);

                if (!this.addedExtra.isEmpty()) {
                    for (int i = 0; i < this.addedExtra.size(); i++) {
                        Service.createExtra(this.addedExtra.get(i).getName(),
                                this.addedExtra.get(i).getPrice(), hotel);
                    }
                }
                ...
                if (EnrollmentAdministration.this.companion.isSelected()) {
                    ...
                    for (int i = 0; i < this.availableExcursions.size(); i++) {
                        for (int j = 0; j < this.addedExcursions.size(); j++) {
                            if (this.conference.getExcursions().get(i)
                                    .getName() == this.addedExcursions.get(j).getName()) {
                ...
                if (EnrollmentAdministration.this.hotel.isSelected()) {
        ...
        private void updateControlls()
        {
            ...
            if (!this.conference.getHotels().isEmpty()) {
                EnrollmentAdministration.this.cmbHotel.getItems()
                        .addAll(this.conference.getHotels());
            }
        }

        // Disables or enables excursions on CheckBox companion click
        ...
        // Disables or enables extras on CheckBox hotel click
        private void enableHotel(MouseEvent event)
        {
            if (EnrollmentAdministration.this.hotel.isSelected()) {
                EnrollmentAdministration.this.cmbHotel.setDisable(false);
                EnrollmentAdministration.this.extras.setDisable(false);
                EnrollmentAdministration.this.addedExtras.setDisable(false);
            } else {
                EnrollmentAdministration.this.cmbHotel.setDisable(true);
                EnrollmentAdministration.this.extras.setDisable(true);
                EnrollmentAdministration.this.addedExtras.setDisable(true);
            }
        }

        // Moves excursion to AddedExcursion ListView
        private void addExcursionAction()
        {
            Excursion excursion = EnrollmentAdministration.this.excursions.getSelectionModel()
                    .getSelectedItem();
            int index = EnrollmentAdministration.this.excursions.getSelectionModel()
                    .getSelectedIndex();
            if (excursion == null) {
                return;
            }
        ...
        // Removes excursions from the AddedExcrsion Listview
        private void removeExcursionAction()
        {
            Excursion excursion = EnrollmentAdministration.this.addedExcursions.getSelectionModel()
                    .getSelectedItem();
            int index = EnrollmentAdministration.this.addedExcursions.getSelectionModel()
                    .getSelectedIndex();
            if (excursion == null) {
                return;
            }
        ...
        // Add the selected extra to the AddedExtra ListView
        private void addExtraAction()
        {
            Extra extra = EnrollmentAdministration.this.extras.getSelectionModel()
                    .getSelectedItem();
            if (extra == null) {
                return;
            }
        ...
        // Deletes the extra from the AddedExtra ListView
        private void deleteExtraAction()
        {
            Extra extra = EnrollmentAdministration.this.addedExtras.getSelectionModel()
                    .getSelectedItem();
            if (extra == null) {
                return;
            }
            this.addedExtra.remove(extra);
            EnrollmentAdministration.this.addedExtras.getItems().clear();
            EnrollmentAdministration.this.addedExtras.getItems().addAll(this.addedExtra);
        }

        // Updates extras once different hotel in ComboBox is selected
        ...
        private boolean checkinput()
        {
            ...
            if (EnrollmentAdministration.this.dateOfArrival.getValue() == null
                    || EnrollmentAdministration.this.dateOfDeparture.getValue() == null) {
                EnrollmentAdministration.this.lblError
                        .setText("Please select the dates for departure and arrival");
                return false;
            }
            ...
            if (arrival.isBefore(this.conference.getStartDate())) {
                EnrollmentAdministration.this.lblError.setText("Dates has to match the conference");
                return false;
            }

            if (arrival.isAfter(this.conference.getEndDate())) {
                EnrollmentAdministration.this.lblError.setText("Dates has to match the conference");
                return false;
            }

            if (arrival.equals(this.conference.getEndDate())) {
                EnrollmentAdministration.this.lblError.setText("Dates has to match the conference");
                return false;
            }
